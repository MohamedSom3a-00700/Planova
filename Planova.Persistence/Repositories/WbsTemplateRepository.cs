...
public class WbsTemplateRepository : IWbsTemplateRepository
{
    ...
    public WbsTemplateRepository(PlanovaDbContext context)
    {
        _context = context;
    }
    ...
    {
        if (_seeded) return;
        if (await _context.WbsTemplates.AnyAsync(ct))
        {
            _seeded = true;
            return;
        }
    ...
    public async Task<WbsTemplate> GetByIdAsync(Guid id, CancellationToken ct)
    {
        await SeedIfEmptyAsync(ct);
        return await _context.WbsTemplates
            .Include(t => t.Items)
            .FirstOrDefaultAsync(t => t.Id == id, ct)
            ?? throw new KeyNotFoundException($"WbsTemplate {id} not found");
    }

    public async Task<IReadOnlyList<WbsTemplate>> GetAllAsync(CancellationToken ct)
    {
        await SeedIfEmptyAsync(ct);
        return await _context.WbsTemplates
    ...
    public async Task<IReadOnlyList<WbsTemplate>> GetByCategoryAsync(string category, CancellationToken ct)
    {
        await SeedIfEmptyAsync(ct);
        return await _context.WbsTemplates
    ...
    public async Task<WbsTemplate> AddAsync(WbsTemplate template, CancellationToken ct)
    {
        _context.WbsTemplates.Add(template);
        await _context.SaveChangesAsync(ct);
        return template;
    }

    public async Task<WbsTemplate> UpdateAsync(WbsTemplate template, CancellationToken ct)
    {
        _context.WbsTemplates.Update(template);
        await _context.SaveChangesAsync(ct);
        return template;
    }
    ...
    {
        var entity = await _context.WbsTemplates.FindAsync(new object[] { id }, ct);
        if (entity is not null)
        {
            _context.WbsTemplates.Remove(entity);
            await _context.SaveChangesAsync(ct);
        }
    }
    ...
    private static WbsTemplate CreateCommercialComplexTemplate()
    {
        var template = new WbsTemplate
        {
            Id = Guid.NewGuid(), Name = "Commercial Complex", Category = "Building Construction", Industry = "Construction",
            Description = "WBS for commercial building projects including retail spaces, offices, common areas, and parking",
            IsStandard = true, Version = 1, Tags = "[\"commercial\", \"building\", \"retail\", \"office\", \"mall\"]",
            CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
        };
        var itemId = Guid.NewGuid;
        var i1 = itemId(); var i1a = itemId(); var i1b = itemId();
    ...
    private static WbsTemplate CreateInfrastructureTemplate()
    {
        var template = new WbsTemplate
        {
            Id = Guid.NewGuid(), Name = "Infrastructure Project", Category = "Infrastructure", Industry = "Civil Engineering",
            Description = "WBS for infrastructure projects including roads, bridges, utilities, and earthworks",
            IsStandard = true, Version = 1, Tags = "[\"infrastructure\", \"civil\", \"road\", \"bridge\", \"utility\"]",
            CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
        };
        var itemId = Guid.NewGuid;
        var i1 = itemId(); var i1a = itemId(); var i1b = itemId();
    ...
    private static WbsTemplate CreateRoadsAndBridgesTemplate()
    {
        var template = new WbsTemplate
        {
            Id = Guid.NewGuid(), Name = "Roads & Bridges", Category = "Infrastructure", Industry = "Civil Engineering",
            Description = "WBS for road and bridge construction including earthworks, pavement, structures, and traffic systems",
            IsStandard = true, Version = 1, Tags = "[\"road\", \"bridge\", \"highway\", \"infrastructure\", \"transportation\"]",
            CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
        };
        var itemId = Guid.NewGuid;
        var i1 = itemId(); var i1a = itemId(); var i1b = itemId();
    ...
    private static WbsTemplate CreateWaterTreatmentTemplate()
    {
        var template = new WbsTemplate
        {
            Id = Guid.NewGuid(), Name = "Water Treatment Plant", Category = "Infrastructure", Industry = "Civil Engineering",
            Description = "WBS for water treatment plant construction including intake, treatment processes, storage, and distribution",
            IsStandard = true, Version = 1, Tags = "[\"water\", \"treatment\", \"plant\", \"infrastructure\", \"utility\"]",
            CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
        };
        var itemId = Guid.NewGuid;
        var i1 = itemId(); var i1a = itemId(); var i1b = itemId();
    ...
    private static WbsTemplate CreateIndustrialTemplate()
    {
        var template = new WbsTemplate
        {
            Id = Guid.NewGuid(), Name = "Industrial Facility", Category = "Industrial", Industry = "Industrial Engineering",
            Description = "WBS for industrial facility construction including process areas, utilities, and commissioning",
            IsStandard = true, Version = 1, Tags = "[\"industrial\", \"facility\", \"process\", \"manufacturing\"]",
            CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
        };
        var itemId = Guid.NewGuid;
        var i1 = itemId(); var i1a = itemId(); var i1b = itemId();
    ...
    private static WbsTemplate CreatePowerPlantTemplate()
    {
        var template = new WbsTemplate
        {
            Id = Guid.NewGuid(), Name = "Power Plant", Category = "Industrial", Industry = "Energy",
            Description = "WBS for power plant construction including generation equipment, electrical systems, cooling, and fuel handling",
            IsStandard = true, Version = 1, Tags = "[\"power\", \"plant\", \"energy\", \"generation\", \"electrical\"]",
            CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
        };
        var itemId = Guid.NewGuid;
        var i1 = itemId(); var i1a = itemId(); var i1b = itemId();
    ...
    private static WbsTemplate CreateOilAndGasTemplate()
    {
        var template = new WbsTemplate
        {
            Id = Guid.NewGuid(), Name = "Oil & Gas Project", Category = "Oil & Gas", Industry = "Energy",
            Description = "WBS for oil and gas projects including well pads, pipelines, processing, and safety systems",
            IsStandard = true, Version = 1, Tags = "[\"oil\", \"gas\", \"energy\", \"pipeline\", \"offshore\"]",
            CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
        };
        var itemId = Guid.NewGuid;
        var i1 = itemId(); var i1a = itemId(); var i1b = itemId();
    ...
    private static WbsTemplate CreateTelecommunicationsTemplate()
    {
        var template = new WbsTemplate
        {
            Id = Guid.NewGuid(), Name = "Telecommunications Network", Category = "Infrastructure", Industry = "Telecommunications",
            Description = "WBS for telecom network deployment including fiber optics, towers, equipment, and commissioning",
            IsStandard = true, Version = 1, Tags = "[\"telecom\", \"fiber\", \"network\", \"tower\", \"communication\"]",
            CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow
        };
        var itemId = Guid.NewGuid;
        var i1 = itemId(); var i1a = itemId(); var i1b = itemId();
