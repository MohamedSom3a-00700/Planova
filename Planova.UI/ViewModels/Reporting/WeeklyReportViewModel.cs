...
{
    private readonly IReportDataProvider<WeeklyReportDataDto> _dataProvider;
    ...
    public WeeklyReportViewModel(
        IReportDataProvider<WeeklyReportDataDto> dataProvider,
    ...
    [ObservableProperty]
    private WeeklyReportDataDto? _reportData;
    ...
    private async Task GenerateReportAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            HasData = true;
            OnStatusMessage?.Invoke($"Weekly report generated for week of {start:d}");
        }
        catch (Exception ex)
        {
    ...
    private async Task GenerateAiNarrativeAsync(CancellationToken ct)
    {
        ...
        try
        {
            AiNarrative = await _aiService.GenerateNarrativeAsync(ReportType.Weekly, ReportData, ct);
    ...
    private async Task ExportAsync(Func<Guid, object, CancellationToken, Task<byte[]>> exportFunc, CancellationToken ct)
    {
        try
        {
            ...
            var start = SelectedWeekStart.Date;
            var end = start.AddDays(6).AddHours(23).AddMinutes(59).AddSeconds(59);
            var instance = await _reportEngine.GenerateAsync(1, ReportType.Weekly, start, end, ct);
            ...
            var fileName = $"{SanitizeFileName(projectName)}_Weekly_{start:yyyy-MM-dd}{extension}";
            ...
            var saveDir = Path.Combine(reportsFolder, "Weekly", dateFolder);
