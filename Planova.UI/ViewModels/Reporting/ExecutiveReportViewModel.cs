...
public partial class ExecutiveReportViewModel : ObservableObject
{
    private readonly IReportDataProvider<ExecutiveReportDataDto> _dataProvider;
    ...
    public ExecutiveReportViewModel(
        IReportDataProvider<ExecutiveReportDataDto> dataProvider,
        ...
        ISettingsService settings)
    {
        ...
        _settings = settings;
        PeriodStart = new DateTime(DateTime.Today.Year, DateTime.Today.Month, 1).AddMonths(-3);
    ...
    [ObservableProperty]
    private ExecutiveReportDataDto? _reportData;
    ...
    private async Task GenerateReportAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var end = PeriodEnd.Date.AddHours(23).AddMinutes(59).AddSeconds(59);
            ReportData = await _dataProvider.CollectDataAsync(1, start, end, ct);
            HasData = true;
            OnStatusMessage?.Invoke($"Executive report generated for {start:yyyy-MM-dd} to {end:yyyy-MM-dd}");
        }
        catch (Exception ex)
        {
    ...
    private async Task GenerateAiNarrativeAsync(CancellationToken ct)
    {
        ...
        try
        {
            AiNarrative = await _aiService.GenerateNarrativeAsync(ReportType.Executive, ReportData, ct);
    ...
    private async Task ExportAsync(Func<Guid, object, CancellationToken, Task<byte[]>> exportFunc, CancellationToken ct)
    {
        try
        {
            ...
            var start = PeriodStart.Date;
            var end = PeriodEnd.Date.AddHours(23).AddMinutes(59).AddSeconds(59);
            var instance = await _reportEngine.GenerateAsync(1, ReportType.Executive, start, end, ct);
            ...
            var fileName = $"{SanitizeFileName(projectName)}_Executive_{start:yyyy-MM-dd}_{end:yyyy-MM-dd}{extension}";
            ...
            var saveDir = Path.Combine(reportsFolder, "Executive", dateFolder);
